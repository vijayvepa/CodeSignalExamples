...
public class DemoApplication implements CommandLineRunner {
  ...
  public void run(String... args) throws Exception {
    ...
    log.info("## ZigZag Traversal \n");
    log.info("Matrix : {}", Arrays.deepToString(matrix));
    int[] expectedZigZag = {12, 8, 4, 3, 7, 11, 10, 6, 2, 1, 5, 9};
    int[] actualZigZag = DimensionalArrays.zigZagTraversal(matrix);

    log.info("Expected Zig Zag: {}", Arrays.toString(expectedZigZag));
    int[] zigZagLearn = DimensionalArrays.zigZagLearn(matrix);
    log.info("Zig Zag Learn: {}", Arrays.toString(zigZagLearn));

    log.info("Actual Zig Zag: {}", Arrays.toString(actualZigZag));

    int[] zigZagLookup = DimensionalArrays.zigZagLambda(matrix);
    log.info("Zig Zag Lookup: {}", Arrays.toString(zigZagLookup));
